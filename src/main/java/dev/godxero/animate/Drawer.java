...
public class Drawer {
	...
	private final double lineJoinDistance;
	...
	public void draw (Graphics g) {
		...
		this.particles.forEach(particle1 -> this.particles.forEach(particle2 -> {
			...
			if (disSquared > this.lineJoinDistance * this.lineJoinDistance) return;

			final int alpha = 255 - (int) (Math.sqrt(disSquared) * 255 / this.lineJoinDistance);

			g.setColor(new Color(255, 255, 255, alpha));
			g.drawLine((int) (particle1.position.x + particle1.size * 0.5), (int) (particle1.position.y + particle1.size * 0.5), (int) (particle2.position.x + particle2.size * 0.5), (int) (particle2.position.y + particle2.size * 0.5));
		}));
	...
	public void update () {
		...
		for (int step = 0; step < subSteps; step++) {
			this.particles.forEach(particle1 -> this.particles.forEach(particle2 -> {
				...
				if (disSquared > radius * radius) return;
